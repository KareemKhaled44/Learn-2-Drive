...
const Profile = () => {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-white text-2xl font-bold">Academy Profile</h2>
                <p className="text-gray-400 text-sm">Edit academy information, upload logo, and manage contacts.</p>
            </div>

            <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
                <div>
                    <label className="text-gray-400 text-sm block mb-1">Name</label>
                    <input name="name" value={profile.name} onChange={handleChange} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white text-sm" />
                </div>

                <div>
                    <label className="text-gray-400 text-sm block mb-1">Description</label>
                    <textarea name="description" value={profile.description} onChange={handleChange} rows={3} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white text-sm resize-none" />
                </div>

                <div>
                    <label className="text-gray-400 text-sm block mb-1">Address</label>
                    <input name="address_text" value={profile.address_text} onChange={handleChange} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white text-sm" />
                </div>

                <div>
                    <label className="text-gray-400 text-sm block mb-1">Google Maps URL</label>
                    <input name="google_maps_url" value={profile.google_maps_url} onChange={handleChange} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white text-sm" />
                </div>
                ...
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <input type="file" accept="image/*" onChange={handleLogoChange} className="text-sm text-gray-300" />
                    </div>
                </div>

                <div className="flex items-center gap-3">
                    <button type="submit" disabled={loading} className="px-4 py-2 bg-cyan-500 rounded-lg text-white">Save Profile</button>
                </div>

                {errors && <div className="text-red-400 text-sm">{typeof errors === 'string' ? errors : JSON.stringify(errors)}</div>}
            </form>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
                ...
                <div className="space-y-3">
                    {contacts.length === 0 ? (
                        ...
                        <div key={c.id} className="flex items-center justify-between bg-gray-800 rounded-lg p-3">
                            ...
                            <div>
                                <button onClick={() => deleteContact(c.id)} className="px-3 py-1.5 rounded-lg bg-red-600 text-xs text-white">Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <form onSubmit={addContact} className="mt-4 flex items-center gap-3">
                    <select name="type" value={newContact.type} onChange={handleNewContactChange} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white">
                        {contactTypes.map(ct => <option key={ct.value} value={ct.value}>{ct.label}</option>)}
                    </select>
                    <input name="value" value={newContact.value} onChange={handleNewContactChange} placeholder="Contact value" className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-sm text-white" />
                    <button type="submit" className="px-4 py-2 bg-cyan-500 rounded-lg text-white">Add</button>
                </form>
            </div>
        </div>
    )
